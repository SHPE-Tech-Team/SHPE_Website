...
// Hardcoded for the script execution since we can't easily rely on Next.js env loading in a standalone script
const client = createClient({
    projectId: 'o87x58mp',
    dataset: 'production',
    apiVersion: '2024-01-01',
    token: process.env.SANITY_API_TOKEN, // We will need a token for write access
    useCdn: false,
})
...
async function migrate() {
    ...
    for (const committee of committees) {
        const doc = {
            ...
            shortDescription: committee.shortDescription,
            // Convert simple string to Portable Text block
            fullDescription: [
                {
                    _type: 'block',
                    ...
                    children: [
                        {
                            _type: 'span',
                            text: committee.fullDescription
                        }
                    ],
                    markDefs: [],
                    style: 'normal'
        ...
        try {
            ...
            console.log(`Created committee: ${committee.title}`)
        } catch (err) {
            console.error(`Failed to create ${committee.title}:`, err)
        }
    }
